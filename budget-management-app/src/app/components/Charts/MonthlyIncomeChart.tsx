...
export default function MonthlyIncomeChart() {
  ...
  const incomesByCategory = currentMonthIncomes.reduce(
  ...
  const filteredCategories = Object.entries(incomesByCategory)
    .filter(([_, amount]) => amount > 0)
    .sort((a, b) => b[1] - a[1]); // Tutara göre azalan sıralama

  const totalIncomes = filteredCategories.reduce(
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
        ...
        <div className="text-center text-gray-500 dark:text-gray-400 py-8">
          Bu ay için henüz gelir kaydı bulunmamaktadır.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg h-full">
      ...
      <div className="mt-4 text-center text-sm text-gray-600 dark:text-gray-300">
        Toplam Gelir: ₺{totalIncomes.toLocaleString()}
      </div>
    </div>
  );
}
